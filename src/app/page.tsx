...
const EditorialCard = ({ image, title, author, cta }: { image: string; title: string; author: string; cta: string }) => (
  <div className="editorial-card relative rounded-md overflow-hidden h-[34rem] bg-white shadow-lg border border-secondary/10">
    ...
    <div className="card-overlay" style={{ position: 'static', padding: '0.75rem', width: '100%' }}>
      ...
      <button className="inline-block mt-2 px-4 py-1 bg-secondary text-primary font-medium rounded shadow hover:bg-accent transition" style={{ fontSize: '1rem' }}>{cta}</button>
    </div>
  </div>
);

const MoodboardCard = ({ image, title, author }: { image: string; title: string; author: string }) => (
  <div className="relative rounded bg-white shadow overflow-hidden h-56 border border-secondary/10">
...
const ProductCard = ({ image, name, brand, price, cta }: { image: string; name: string; brand: string; price: string; cta: string }) => (
  <div className="relative rounded bg-white shadow overflow-hidden h-56 border border-secondary/10">
    ...
    <div className="card-overlay">
      ...
      <div className="card-price text-secondary">{price}</div>
      <button className="inline-block mt-2 px-3 py-1 bg-secondary text-primary font-medium rounded hover:bg-accent transition" style={{ fontSize: '0.9rem' }}>{cta}</button>
    </div>
  </div>
);

const DesignerCard = ({ image, name, specialty, cta }: { image: string; name: string; specialty: string; cta: string }) => (
  <div className="relative rounded bg-white shadow overflow-hidden h-56 flex flex-col items-center justify-end border border-secondary/10">
    ...
    <div className="card-overlay" style={{ position: 'static', padding: '0.75rem', width: '100%' }}>
      ...
      <button className="inline-block mt-2 px-4 py-1 bg-accent text-primary font-medium rounded hover:bg-secondary transition" style={{ fontSize: '0.9rem' }}>{cta}</button>
    </div>
  </div>
);
...
export default function Home() {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-primary">
      ...
      {/* Main Content */}
      <div className="flex-1 lg:ml-20 xl:ml-56 overflow-y-auto bg-primary">
        {/* Top Bar with Search */}
        <div className="w-full flex items-center justify-end mt-4 px-4">
          <div className="relative w-full max-w-xs">
            <input
              type="text"
              placeholder="Search..."
              className="w-full pl-10 pr-4 py-2 rounded-md bg-white border border-secondary/20 text-secondary placeholder-text-light focus:outline-none focus:ring-2 focus:ring-secondary focus:border-transparent"
              value={searchQuery}
        ...
        {/* Filter Controls */}
        <div className="w-full flex items-center justify-between mt-2 mb-4 px-2">
          <div className="relative">
            <button className="flex items-center bg-white border border-secondary/20 text-secondary px-4 py-2 rounded-md font-medium min-w-[120px] opacity-60 cursor-not-allowed">
              <span>For You</span>
              <FaChevronLeft className="ml-2 text-xs" />
            </button>
          </div>
          <div className="flex-1 flex justify-center gap-6">
            <button className="feed-subtab px-4 py-2 font-medium text-secondary rounded-md opacity-60 cursor-not-allowed">All</button>
            <button className="feed-subtab px-4 py-2 font-medium text-secondary rounded-md opacity-60 cursor-not-allowed">Projects</button>
            <button className="feed-subtab px-4 py-2 font-medium text-secondary rounded-md opacity-60 cursor-not-allowed">Products</button>
          </div>
      ...
      {showModal && selectedCard && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-secondary bg-opacity-80">
          <div className="bg-primary rounded-md shadow-lg flex flex-col md:flex-row max-w-3xl w-full mx-4 relative border border-secondary/20">
            <button
              className="absolute top-3 right-3 text-primary text-2xl bg-secondary bg-opacity-80 rounded-full w-10 h-10 flex items-center justify-center z-10 hover:bg-opacity-100 transition"
            ...
            <div className="md:w-1/2 w-full flex flex-col justify-center p-6">
              <h2 className="text-lg font-semibold mb-2 text-secondary">{selectedCard.title || selectedCard.name}</h2>
