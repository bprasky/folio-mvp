...
async function detectAuth(): Promise<AuthSummary> {
  try {
    ...
    try {
      ...
      const errorSummary: AuthSummary = {
        prismaUsers: 0,
        hasAdmin: false,
        usingSupabaseAuth: false,
        databaseConnected: false,
    ...
    const adminEmails = adminUsers.map(user => user.email).filter((email): email is string => email !== null);
